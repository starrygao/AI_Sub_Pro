...
- macOS 用户可以从 `v1.3.2` release 资产下载 `AI_Sub_Pro_v1.3.2.dmg` 和
  `AI_Sub_Pro_v1.3.2.dmg.sha256`，运行
  `shasum -a 256 -c AI_Sub_Pro_v1.3.2.dmg.sha256` 校验后打开 DMG，将
...
默认安装包会在构建环境存在可用后端时包含 ASR backend package / ASR 后端包，但
不包含本地 Whisper 模型文件。面向离线使用的大包需要显式设置
`AISUBPRO_BUNDLE_LOCAL_ASR=1` 构建，并在 release notes 中标注为 optional ASR
model package。维护者只有在明确要制作无 ASR 小包时，才应设置
`AISUBPRO_BUNDLE_ASR_BACKENDS=0`。
...
源码运行时，可以在本地 Python 环境中安装可选 ASR 后端。打包版默认会打包已安装
的 ASR 后端包；只有在维护者设置 `AISUBPRO_BUNDLE_LOCAL_ASR=1` 时，才会把
